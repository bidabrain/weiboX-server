...
services:
  weibox-server:
    ...
    ports:
      # 只绑回环，仅本机可访问。想让局域网里的手机直连，改成 "8000:8000"
      - "127.0.0.1:8000:8000"
    volumes:
      - ./data:/data
    environment:
...
> ⚠️ **端口绑定要按你的访问方式选，否则手机会连不上。**
> `ports` 的格式是 `宿主机IP:宿主机端口:容器端口`，最前面那个 IP 决定绑在哪张网卡上：
>
> | 写法 | 谁能访问 |
> |---|---|
> | `"127.0.0.1:8000:8000"` | **只有服务器本机**。适合配合 Cloudflare Tunnel——`cloudflared` 连 `localhost:8000` 即可，端口不暴露，少一个被扫描的入口 |
> | `"8000:8000"` | 所有网卡，**同局域网的设备可直连**。若服务器有公网 IP 或路由器做了端口转发，公网也能访问 |
>
> 上面的默认值是回环绑定。**如果你打算用局域网 IP（`http://192.168.x.x:8000`）连 app，
> 必须改成 `"8000:8000"`**，改完 `docker compose up -d` 重建容器。
